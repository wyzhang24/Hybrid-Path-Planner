  ...
  public:
    vector <vector< double > > calc_dist_policy(double gx, double gy, vector<double>* oox, vector<double>* ooy, double reso, double vr){
        ...
        */
        ...
        //cout<<"obstacle map finished"<<endl;
        ...
        while (1){
            ...
            for (size_t i = 0; i < nmotion; i++){ // expand search grid based on motion model
                ...
                if (closed_set.find(node_ind) != closed_set.end()) continue;
        ...
        //cout<<"start calc policy map"<<endl;
    ...
    vector <vector< double > > calc_policy_map(unordered_map<int, node_s>* closed) {
        ...
        // cout << "comeout:::: " << endl;
        //cout<<"pmap size"<<pmap.size()<<' '<<pmap[0].size()<<endl;
        for (auto it = closed->begin(); it != closed->end(); ++it){
            node_s n = it->second;
            //cout << "x: " << n.x - minx -1 << endl;
            //cout<<"minx = "<<minx<<endl;
        ...
        //cout<<"end calculate policy map"<<endl;
        return pmap;
    }
    ...
    // build an obstacle map and in each grid, it shows whether it has obstacle
        cout << "comeindis: " << ox->size() << endl;
        auto ox_min_max = minmax_element(ox->begin(), ox->end());
        auto oy_min_max = minmax_element(oy->begin(), oy->end());
        //cout<<"resolution = "<<reso<<endl;
        // the obstacle map cannot be an empty one (otherwise lead to dereference of a null ptr)
        ...
        cout<< "minx = "<< *(ox_min_max.first)<<endl;
        cout<< "reslution = "<<reso<<endl;
        xw = maxx - minx;
        //cout<<"xw = "<<xw<<endl;
        yw = maxy - miny;
        //cout<<"yw = "<<yw<<endl; 
        ...
        //cout<<"end obstacle calculaion"<<endl;
    }
