...
public:
  ...
  double max_steering = 50.0 * PI / 180.0; //
  double coeff = 1.0;
  double v_reso = 0.01;
  ...
  double oscillation_cost_gain = 0.001;
  ...
  // direction = true if moving forward, false otherwise
  void segmentPath() {
    pair<bool, vector<pair<double, double>>> tmpPath(direction[0], vector<pair<double, double>>());
    pathDir = direction[0];
    for(int i = 1; i < path.size(); ++i) {
      if(direction[i] != direction[i - 1]) {
        pathVec.push_back(tmpPath);
        tmpPath = pair<bool, vector<pair<double, double>>>(direction[i], vector<pair<double, double>>());
      }
      else {
...
*/
  ...
  // dist -- [0, pi]
  cost = traj.back()[3] > 0 ? dist * PI / 180 : (180 - dist) * PI / 180;
  // cost = dist * PI / 180;
  return 0.15 * cost;
...
double calc_grid_cost(Traj traj, Config& config) {
  ...
  Point goalPoint = idx >= config.path.size() - 5 ? Point{config.path.back().first, config.path.back().second}
   : Point{config.path[idx + 5].first, config.path[idx + 5].second};
  cost = calc_to_goal_cost(traj, goalPoint, config);
  Point secPoint = idx >= config.path.size() - 1 ? Point{config.path.back().first, config.path.back().second}
   : Point{config.path[idx + 1].first, config.path[idx + 1].second};
  cost += distCalc(traj[0][0], traj[0][1], secPoint[0], secPoint[1]);
  // cout << "goal point is" << goalPoint[0] << " " << goalPoint[1] << endl;
  return cost;
}
...
double calc_oscillation_cost(State x, Traj traj, Config config) {
  ...
  oscillate_cost += fabs(traj.back()[2]); // penalize large steering
...
double calc_speed_cost(Traj traj, Config config) {
  ...
  double cost = 0;
  if(speed < 0 && config.pathDir) cost += config.reverseCost;
  ...
  std::vector<std::array<double, 2>>ob) {
    ...
    for (double v=dw[0]; v<=dw[1]; v+=config.v_reso){
      for (double y=dw[2]; y<=dw[3]; y+=config.yawrate_reso){
        ...
        double final_cost = to_goal_cost + speed_cost + ob_cost + grid_cost + oscillation_cost;
...
Traj dwa_control(State x, Control & u, Config & config,
  Point goal, Obstacle ob){
    ...
    if(sqrt(distCalc(x[0], x[1], config.path.back().first, config.path.back().second)) < config.robot_radius) {
